...
Why webscrape?
While some influential people consider "Data Scientist" `r emo::ji("woman_technologist")` to be the [sexiest job](https://hbr.org/2012/10/data-scientist-the-sexiest-job-of-the-21st-century) of the 21st century (congratulations!), one of the sexiest just emerging academic disciplines is (in my influential view) - Computational Social Science (CSS).
Why is that so?
...
For many APIs, you will need to obtain an api key to retrieve data.
Once you received your api key (or token), you will also need to adapt your GET query.
How you need to do this depends a lot on the API.
The National Renewable Data Laboratory offers APIs for users to access energy data in a few categories (see [documentation](https://developer.nrel.gov/docs/)).
The API is actually valid across a number of [US government institutions](https://api.data.gov/).
We can sign up for an API key [here](https://api.data.gov/signup/).
The API website, gives us detailed information on how to build our queries.
But how do we actually authenticate ourselves with the API key?
The [documentation](https://api.data.gov/docs/api-key/) tells us that there are multiple ways to go about this.
We can either do adapt the use HTTP basic authentication, the GET query parameter, or the HTTP header.

Here's a quick overview of how to implement these with `httr`:

```{r, eval=FALSE}
baseurl <- 'https://developer.nrel.gov/api/alt-fuel-stations/v1.json?limit=1&api_key='

api_key <- '[YOUR API KEY GOES HERE]'

# Get stations data in the US, remember to plug in your own API key
get.afs <- GET(glue::glue("{baseurl}{api_key}"))
...
# how many stations per energy type
fuel_list <- afs$station_counts$fuels
...
fuel_df <- fuel_list |> 
  ...
  mutate(fuel_type = fct_reorder(fuel_type, desc(count))) # reorder 

# plot type of alternative fuel the station provides
fuel_df |> 
  ggplot(aes(x = fuel_type, y = count)) +
  geom_col(fill="steelblue") + 
  theme_minimal() 
```

Retrieving data from API's using `httr` can at times be quite tiresome.
Luckily, there are many R libraries that make it much easier to retrieve data from APIs.
Here is a [list of ready-made R bindings to web-APIs](https://ropensci.org/packages/data-access/).
Actually, even the starwars API we queried earlier has its own R package, `rwars`!
...
We also used an [example](https://keith-mcnulty.medium.com/how-to-scrape-the-web-in-r-d77b11fca40d) from Keith McNulty's blog post on tidy web scraping in R and another [example](https://library.virginia.edu/data/articles/using-data-gov-apis-in-r) from Yun Tai's article on using Data.gov APIs in R.
For the regex part, we used examples from the string manipulation section in Hadley Wickham's [R for Data Science](https://r4ds.hadley.nz/strings) book.
